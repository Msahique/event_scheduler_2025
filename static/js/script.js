...
/************************************************ event dependency ************************************************************    */ 
function updateEvent(eventId, newData, rowData) {
    const oldStart = rowData['Start DateTime'];
    const oldEnd = rowData['End DateTime'];
    const newStart = newData['Start DateTime'];
    const newEnd = newData['End DateTime'];

    // Check if there's any change in Start or End DateTime
    if (oldStart !== newStart || oldEnd !== newEnd) {
        console.log("Start/End time changed. Calling createEvent()...");
        createEvent(eventId, newData);
    } else {
        console.log("No change in time. No need to call createEvent().");
    }
}
